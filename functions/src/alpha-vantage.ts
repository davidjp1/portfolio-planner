...
export async function queryAlphaVantageCsvToJson(symbol: string, avFunction: string, skipCache = false) {
  const cachedValue = !skipCache && await checkCache(symbol, avFunction, '&horizon=12month');
  if(cachedValue){
    return cachedValue;
  }
  const {data: csvData,errors} = parseCsv(await callAlphaVantage(avFunction, symbol, '&horizon=12month'));
...
export async function getCachedOrAlphaVantage(symbol: string, avFunction: string, additionalParams?: string, skipCache = false): Promise<any> {
  const cachedValue = !skipCache && await checkCache(symbol, avFunction, additionalParams);
  if(cachedValue){
    return cachedValue;
  }
...
async function callAlphaVantage(avFunction: string, symbol: string, additionalParams?: string) {
  console.log(`calling alpha vantage for function=${avFunction} symbol=${symbol} additionalParams=${additionalParams}`);
...
async function checkCache(symbol: string, avFunction: string, additionalParams?: string): Promise<any> {
  const document = await db.collection('cache').doc(symbol).collection(avFunction).doc(additionalParams || 'result').get();
  if(!document.exists){
    return null;
  }
...
async function updateCache(symbol: string, avFunction: string, result: any, additionalParams?: string): Promise<void> {
  db.collection('cache').doc(symbol).collection(avFunction).doc(additionalParams || 'result').set({rawData: JSON.stringify(result), dateAdded: new Date()});
}